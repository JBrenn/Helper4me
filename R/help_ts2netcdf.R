...
help_ts2netcdf <- function(ts, unit, latitude, longitude, name.nc)
{
  ...
  RNetCDF::dim.def.nc(ncfile = nc, dimname = "y", dimlength = dim(lat)[1])
  # set x-lon dimension, length: dim(lon) 
  RNetCDF::dim.def.nc(ncfile = nc, dimname = "x", dimlength = dim(lon)[1])
  ...
  RNetCDF::var.put.nc(ncfile = nc, variable = "lat", data = lat)
  # put longitude in degree
  RNetCDF::var.put.nc(ncfile = nc, variable = "lon", data = lon)
